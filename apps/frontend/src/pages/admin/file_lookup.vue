		...
		<div class="normal-page__content">
			<div class="card flex flex-col gap-3">
				...
				<template v-if="lookupResult">
					<h3 class="mb-0 text-lg font-extrabold text-contrast">Blankethub project:</h3>
					<nuxt-link
					...
					<h3 class="mb-0 text-lg font-extrabold text-contrast">Blankethub version:</h3>
					<nuxt-link
...
async function lookupFile(hash: string): Promise<void> {
	...
	try {
		...
		if (error.status === 404) {
			lookupError.value = `File not found on Blankethub across projects you have access to.`
		} else {
