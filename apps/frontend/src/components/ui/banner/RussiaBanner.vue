...
import { ButtonStyled, PagewideBanner, commonMessages, useVIntl } from '@modrinth/ui'
	...
	<PagewideBanner v-if="!flags.hideRussiaCensorshipBanner" variant="error">
		<template #title>
			<div class="flex flex-col gap-1 text-contrast">
				<span lang="ru">К сожалению, Blankethub скоро станет недоступен в России</span>
				<span class="text-sm font-medium opacity-50" lang="en">
					Blankethub will soon be unavailable in Russia
				</span>
			</div>
		</template>
		<template #description>
			<p class="m-0" lang="ru">
				Российское правительство потребовало от нас заблокировать некоторые проекты на Blankethub, но
				мы решили отказать им в цензуре.
			</p>
			<p class="-mt-2 mb-0 text-sm opacity-50" lang="en">
				The Russian government has asked us to censor certain topics on Blankethub and we have decided
			...
			<p class="-mt-2 mb-0 text-sm opacity-50" lang="en">
				Please seek a reputable VPN or proxy of some kind to continue to access Blankethub in Russia.
			</p>
		</template>
		<template #actions>
