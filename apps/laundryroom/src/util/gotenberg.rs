...
impl GotenbergClient {
    ...
    /// then waits until some Laundryroom instance receives a response on the
    /// Gotenberg webhook, sends the response over Redis to our instance, and
    /// returns that from this function.
    ///
    /// In a local environment, the Laundryroom instance that receives the webhook
    ...
    /// Laundryroom instances so we need Redis in between.
    ///
