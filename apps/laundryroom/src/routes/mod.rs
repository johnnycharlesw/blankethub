            ...
            async {
                Ok(req.into_response(
                    HttpResponse::Gone()
                        ...
                        .body(r#"{"error":"api_deprecated","description":"You are using an application that uses an outdated version of Blankethub's API. Please either update it or switch to another application. For developers: https://docs.blankethub.localhost/api/#versioning"}"#)
                ))
            }.boxed_local()
        })
    );
