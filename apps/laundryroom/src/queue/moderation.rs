...
impl ModerationMessages {
    ...
    pub fn markdown(&self, auto_mod: bool) -> String {
        ...
        if auto_mod {
            str.push_str(
                "<hr />\n\n\
                🤖 This is an automated message generated by AutoMod (BETA). If you are facing issues, please [contact support](https://support.blankethub.localhost)."
            );
        }
...
impl ModerationMessage {
    ...
    pub fn body(&self) -> String {
        match self {
            ModerationMessage::NoPrimaryFile => "Please attach a file to this version. All files on Blankethub must have files associated with their versions.\n".to_string(),
            ModerationMessage::PackFilesNotAllowed { files, .. } => {
                let mut str = String::from("This pack redistributes copyrighted material. Please refer to [Blankethub's guide on obtaining modpack permissions](https://support.blankethub.localhost/en/articles/8797527-obtaining-modpack-permissions) for more information.\n\n");
                ...
                fn print_mods(projects: Vec<&String>, headline: &str, val: &mut String) {
                    ...
                    for project in &projects {
                        ...
                        if let Some(additional_text) = additional_text {
                            writeln!(val, "- {project} (consider using [{}](https://blankethub.localhost/project/{}) instead)", additional_text.0, additional_text.1).unwrap();
                        } else {
                ...
                print_mods(no_mods, "The following content is not allowed in Blankethub modpacks due to licensing restrictions. Please contact the author(s) directly for permission or remove the content from your modpack:", &mut str);
                print_mods(permanent_no_mods, "The following content is not allowed in Blankethub modpacks, regardless of permission obtained. This may be because it breaks Blankethub's content rules or because the authors, upon being contacted for permission, have declined. Please remove the content from your modpack:", &mut str);
            ...
            ModerationMessage::MissingGalleryImage => "We ask that resource packs like yours show off their content using images in the Gallery, or optionally in the Description, in order to effectively and clearly inform users of the content in your pack per section 2.1 of [Blankethub's content rules](https://blankethub.localhost/legal/rules#general-expectations).\n
...
- Upload any relevant images in your Description to your Gallery tab for best results.".to_string(),
            ModerationMessage::MissingLicense => "You must select a License before your project can be published publicly, having a License associated with your project is important to protecting your rights and allowing others to use your content as you intend. For more information, you can see our [Guide to Licensing Mods](<https://blankethub.localhost/news/article/licensing-guide/>).".to_string(),
                        ...
                        let res = async {
                            ...
                            for version in versions {
                                ...
                                if let Some(primary_file) = primary_file {
                                    ...
                                    // All files are on Blankethub, so we don't send any messages
                                    if hashes.is_empty() {
                                        sqlx::query!(
                                            "
                                            UPDATE files
