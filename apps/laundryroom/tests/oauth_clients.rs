...
async fn can_create_edit_get_oauth_client() {
    with_test_environment(None, |env: TestEnvironment<ApiV3>| async move {
        ...
        let redirect_uris = vec![
            "https://blankethub.localhost".to_string(),
            "https://blankethub.localhost/a".to_string(),
        ];
        let resp = env
            .api
            .add_oauth_client(
        ...
        let url = Some("https://blankethub.localhost".to_string());
        let description = Some("test description".to_string());
        let edited_redirect_uris = vec![
            redirect_uris[0].clone(),
            "https://blankethub.localhost/b".to_string(),
        ];
    ...
    with_test_environment(None, |env: TestEnvironment<ApiV3>| async move {
        let resp = env
            ...
            .add_oauth_client(
                ...
                Scopes::restricted(),
                vec!["https://blankethub.localhost".to_string()],
                FRIEND_USER_PAT,
            )
            .await;
