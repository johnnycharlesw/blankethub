...
The most popular license on Blankethub, the [MIT License](https://cdn.blankethub.localhost/licenses/mit.txt), is a permissive license. It is an easy-to-read license designed to be used for developers, which is why it is used extensively in the Minecraft open source community.

The [Apache License 2.0](https://cdn.blankethub.localhost/licenses/apache.txt) is also a very good permissive license to use. The main difference between it and the MIT License is that the Apache License gives an explicit patent grant, whereas patents must be registered manually with the MIT. There is also an additional clause with the Apache License, stating that any modified files must "carry prominent notices" of it being modified.
...
The second most common license on Blankethub is a copyleft license: the [GNU Lesser General Public License Version 3](https://cdn.blankethub.localhost/licenses/lgpl-3.txt) (usually [shortened to](https://spdx.org/licenses/LGPL-3.0-only.html) LGPL-3.0).
...
The third most common license used on Blankethub is the [Creative Commons Zero 1.0 Universal](https://cdn.blankethub.localhost/licenses/cc0.txt), which is a public domain dedication with a strong international legal basis, while still retaining trademark and patent rights.

Creative Commons licenses as a whole are not recommended for software, but rather for other creative works: use this license with caution. If you wish to have the simplest public domain dedication possible, the [Unlicense](https://cdn.blankethub.localhost/licenses/unlicense.txt) is also an option.
...
We are not lawyers, and thus, **this is not legal advice.** No warranty is given regarding this information, and we (Blankethub) disclaim liability for damages resulting in using this information given on an "as-is" basis. For more information on the legal aspect to software licensing, please refer to "[The Legal Side of Open Source](https://opensource.guide/legal/)".

No matter your choice of license, by uploading any Content (including but not limited to text, software, and graphics) to Blankethub, you give us certain rights to your Content, including but not limited to the ability to use, reproduce, or distribute. For more information, please see the [Blankethub Terms of Use](https://blankethub.localhost/legal/terms).
